...
export default function Login({ onSwitchToSignup, onLoginSuccess }) {
  ...
  const wrapRef   = useRef(null);   // the flex:1 area that sizes the canvas

  /* all game state — mutated directly, never setState in hot path */
  const g = useRef({
    W:0, H:0, GY:0,
    ...
    groundOff:0, dayT:1.0,   /* start at day (sin=1 → day=1) */
    phase:'running',          /* running | dead */
    restartTimer:0,
    raf:null,
  }).current;                 /* .current once — we mutate g directly */
  ...
  /* resize observer — sets canvas resolution = CSS size */
  useEffect(()=>{
      ...
      if(g.phase==='running'){
        ...
        for(const o of g.obstacles){
          if(cx1<o.x+o.w-3&&cx2>o.x+3&&cy1<o.y+o.h-3&&cy2>o.y+3){
            ...
            g.restartTimer=150; /* ~2.5s at 60fps */
            break;
          }
        }
        ...
        /* ── DEAD ── draw frozen scene then auto-restart */
        for(const o of g.obstacles){
      ...
      <div className="login-left">
        <div className="left-inner">
          ...
          <div className="headline">
            <h1>Spark your<br/><em>creativity</em></h1>
          </div>

          <p className="tagline">
            Your most capable AI assistant.<br/>Think deeper, create faster.
          </p>

          <div className="auth-card">
            <p className="auth-card-title">
              Start using Horizon for yourself or your team
            </p>
            ...
            <input
              ...
              onKeyDown={e => e.key==='Enter' && email.trim() && password.trim() && handleLogin()}
            />
